...
每個版本一個區塊，最新的排最上面、預設展開，其餘收合。展開後是**左右兩欄**：

- **✨ 新功能（左欄）**：只收 Jira 的 **Epic**。顯示 功能名、可點的 Jira 單號、白話說明、影響。
- **🐛 Bug 修復（右欄）**：收 Jira 的 **Bug** 與 **Stage-bug**（規則見 CLAUDE.md）。顯示 問題名、Jira 單號、原本狀況、修好後。
- **⚙️ 後端／設定調整（免更新 App）**：收貼了 `release-note-backend` label 的單，放在兩欄下方整寬一條，沒資料就不顯示。

兩欄固定同高（520px），內容超出在欄內滾動，不會被另一欄撐高。手機寬度自動改成上下堆疊。

**搜尋**：頁面最上方是全站搜尋（跨版本找「這個 Bug 是哪版修的」，有命中的版本自動展開、沒命中的隱藏）；每個版本展開後另有版本內搜尋。兩者都比對標題、白話／影響／原本／修好後全文與單號，命中的字會反白。

> Task（RD 實作子單、埋點）**不收進對外看板**，避免雜訊。
...
1. 版面改為雙欄：每個版本展開後左欄「新功能」、右欄「Bug 修復」並排，兩欄固定同高 520px、內容超出在欄內滾動，欄底有漸層提示「下面還有」。後端區塊放兩欄下方整寬一條。頁寬由 860 放寬到 1180；760px 以下改上下堆疊。

2. 新增全站搜尋（頁首）與版本內搜尋（各版本展開後）。純前端過濾，比對標題、內文、單號，命中反白、欄標題顯示「命中 / 總數」；全站搜尋會自動展開有命中的版本並隱藏沒命中的，清空回到預設狀態。

3. 版本標頭的「N 新功能 · N Bug」改成帶色塊，對應下方兩欄顏色。

4. `build.js` 支援 `BOARD_OUT=路徑` 指定輸出檔，方便用測試密碼驗版面而不覆蓋正式 `index.html`。
